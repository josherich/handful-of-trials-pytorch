...
from env.manipulator import env
...
class ManipulatorConfigModule:
    ...
    def obs_cost_fn(self, obs):
        ...
        obj = obs[:, 33:35]
        tar = obs[:, 41:43]
        dist = np.linalg.norm(tar - obj, axis=1)

        cost = self._is_close(dist)
...
CONFIG_MODULE = ManipulatorConfigModule
